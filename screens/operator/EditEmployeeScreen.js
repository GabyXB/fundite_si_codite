...
const EditEmployeeScreen = () => {
      ...
      const fetchEmployee = async () => {
        ...
        try {
          ...
          if (resp.ok && isActive) {
            ...
            Alert.alert('Eroare', data.error || 'Eroare la încărcare.');
          }
        } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        Alert.alert('Succes', 'Angajat editat cu succes!');
        navigation.goBack();
      } else {
        const data = await resp.json();
        Alert.alert('Eroare', data.error || 'Eroare la editare.');
      }
    } catch (e) {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      <View style={styles.container}>
        ...
        <Text style={styles.title}>Editează angajat</Text>
        <TextInput
          style={styles.input}
          placeholder="Nume"
          value={nume}
        ...
        <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit}>
          ...
          <Text style={styles.submitBtnText}>Salvează</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
