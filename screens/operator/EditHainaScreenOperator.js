...
const EditHainaScreenOperator = () => {
  ...
  const fetchHaina = async () => {
    ...
    try {
      ...
      setImagineGen(data.imagine_gen || '');
    } catch (e) {
      Alert.alert('Eroare', 'Nu s-au putut încărca detaliile hainei.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let imagineGenUrl = imagine_gen;
      // Dacă imaginea nu e link, o urcăm
      ...
      if (imagine_gen && !imagine_gen.startsWith('http')) {
        setUploadingGen(true);
        const imageFormData = new FormData();
        imageFormData.append('image', {
          uri: imagine_gen,
          type: 'image/jpeg',
          name: 'haina-image-gen.jpg',
        });
        const uploadResponse = await fetch('http://13.60.13.114:5000/api/upload', {
          method: 'POST',
          headers: {
        ...
        if (!uploadResponse.ok) throw new Error('Nu am putut încărca imaginea generată');
        const uploadData = await uploadResponse.json();
        imagineGenUrl = uploadData.imageUrl;
      ...
      const resp = await fetch(`http://13.60.13.114:5000/api/haine/${hainaId}`, {
        ...
        body: JSON.stringify({
          ...
          imagine_gen: imagineGenUrl ? imagineGenUrl : null,
        })
      });
      if (resp.ok) {
        Alert.alert('Succes', 'Haină modificată cu succes!');
        navigation.goBack();
      } else {
        const data = await resp.json();
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      <View style={styles.container}>
        ...
        <Text style={styles.title}>Modifică haină</Text>
        <TextInput
          style={styles.input}
...
export default EditHainaScreenOperator; 